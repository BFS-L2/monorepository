	...
	return (
		<div className='flex w-full flex-col items-center rounded-xl bg-white p-6 text-teal-400 dark:bg-zinc-800'>
			<form onSubmit={handleSubmit} className='flex w-full flex-col gap-2'>
				...
				<span className='ml-1 text-sm text-zinc-800 dark:text-zinc-200'>
					Coin price:{' '}
					<span className='font-mono text-teal-400'>{coinPrice && `${coinPrice} USD`}</span>
				</span>

				<Select options={currencyOptions} value={selectedCoin} onChange={setSelectedCoin} />
